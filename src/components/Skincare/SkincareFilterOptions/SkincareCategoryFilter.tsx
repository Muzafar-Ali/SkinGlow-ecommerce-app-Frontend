...
const SkincareCategoryFilter = ({ skincareCategories, skincareCategoryFilters, handleSkincareFiltersChange }: SkincareCategoryFilterProps) => {
  ...
  return (
    <div className='border-b border-neutral-200 p-4 bg-white'>
      ...
      >
        <h2 className="w-full text-pink-800 text-base font-bold capitalize leading-snug bg-transparent">category</h2>
      ...
      <div className={`${isDropDown ? 'visible-container' :'hidden-container'} flex flex-col gap-y-2 mt-2 bg-white`}>
        { skincareCategories?.map((item, index) => (
          <div 
            className='inline-flex items-center gap-x-2 bg-white' 
            key={index}
          >
            <div 
              className={`${skincareCategoryFilters?.includes((item?._id.toLowerCase()).split(' ').join('-')) ? "bg-neutral-950 ring-2 ring-white ring-inset":"bg-white"} w-4 h-4 border-2 hover:scale-125 border-neutral-950 rounded-sm cursor-pointer`}
              onClick={() => handleSkincareFiltersChange('skincare', (item?._id.toLowerCase()).split(' ').join('-'))}
            />
...
export default SkincareCategoryFilter
