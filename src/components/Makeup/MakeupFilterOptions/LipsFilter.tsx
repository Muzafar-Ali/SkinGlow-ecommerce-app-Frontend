...
type LipsFilterProps = {
  lipCategories: CategoryType[]
  lipsFilters: string[];
  handleMakeupFiltersChange: (filetrTyp: string, value: string) => void;
}

const LipsFilter = ({ lipCategories, lipsFilters, handleMakeupFiltersChange }: LipsFilterProps) => {
  ...
  return (
    <div className='border-b border-neutral-200 p-4 bg-white'>
      ...
      <div className={`${isDropDown ? 'visible-container' :'hidden-container'} flex flex-col gap-y-2 mt-2 bg-white`}>
        { lipCategories?.map((item, index) => (
          <div 
            className='inline-flex items-center gap-x-2 bg-white' 
            key={index}
          >
            <div 
              className={`${lipsFilters?.includes((item?._id.toLowerCase()).split(' ').join('-')) ? "bg-neutral-950 ring-2 ring-white ring-inset":"bg-white"} w-4 h-4 border-2 hover:scale-125 border-neutral-950 rounded-sm cursor-pointer`}
              onClick={() => handleMakeupFiltersChange('lips',(item?._id.toLowerCase()).split(' ').join('-'))}
            />
